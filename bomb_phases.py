...
class Lcd(Frame):
    ...
    def conclusion(self, success=False):
        ...
        # reconfigure the GUI
        if (success):
            image = PhotoImage(file=WINNER_IMAGE)
            pygame.mixer.music.load(WINNER_AUDIO)
            pygame.mixer.music.play(1)
        else:         
            image = PhotoImage(file=LOSER_IMAGE)
            pygame.mixer.music.load(LOSER_AUDIO)
            pygame.mixer.music.play(1)
...
class Wires(PhaseThread):
    ...
    # returns the toggles state as a string
    def __str__(self):
        if (self._defused):
            return "DEFUSED"
        else:
